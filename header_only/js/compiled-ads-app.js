...
(function () {
  ...
  function positionAdsTipElement() {
    ...
    if (!frame) {
      ...
      left = Math.max(
        viewport.left + pad,
        Math.min(left, viewport.left + viewport.width - tipW - pad),
      );
      if (top + tipH > viewport.top + viewport.height - pad) {
        top = Math.max(viewport.top + pad, rect.top - tipH - gap);
      }
    ...
    const metrics =
      typeof window.__getZoomFrameOverlayMetrics === "function"
        ? window.__getZoomFrameOverlayMetrics(frame)
        : null;
    const scale = metrics ? metrics.scale : getZoomFrameScale(frame);
    const anchorRect = anchor.getBoundingClientRect();
    const frameRect = metrics ? metrics.frameRect : frame.getBoundingClientRect();
    ...
    const tipW = adsTipEl.offsetWidth;
    const tipH = adsTipEl.offsetHeight;
    const minLeft = metrics ? metrics.left + pad : pad;
    const maxLeft = metrics ? metrics.right - tipW - pad : frame.offsetWidth - tipW - pad;
    const minTop = metrics ? metrics.top + pad : pad;
    const maxTop = metrics ? metrics.bottom - tipH - pad : frame.offsetHeight - tipH - pad;
    ...
    if (top + tipH > (metrics ? metrics.bottom : frame.offsetHeight) - pad) {
      top = anchorTop - tipH - gap;
    }
  ...
  function showAdsTip(anchor, text, opts) {
    ...
    if (
      !text ||
      !anchor ||
      document.documentElement.classList.contains("is-zoom-hover-blocked")
    )
      return;
    opts = opts || {};
  ...
  function initAdsTipViewportSync() {
    function syncAdsTipPosition() {
      if (document.documentElement.classList.contains("is-zoom-hover-blocked")) {
        hideAdsTip();
        return;
      }
  ...
  /* ---------- Статический рендер UI-текста (движок масок) ----------
     ...
     обычном ctrl+/- не было дёрганья (ТЗ п.5, п.6).
     В Firefox «Только текст» общий движок переключает UI на живой текст:
     для этого режима статического дизайна быть не должно.
