...
function getZoomFrameOverlayMetrics(frame) {
  if (!frame) return null;

  const frameRect = frame.getBoundingClientRect();
  const scale =
    frameRect.width && frame.offsetWidth ? frameRect.width / frame.offsetWidth : 1;
  ...
  return {
    ...
    scale: safeScale,
    viewport,
    left: (viewport.left - frameRect.left) / safeScale,
    top: (viewport.top - frameRect.top) / safeScale,
    right: (viewport.left + viewport.width - frameRect.left) / safeScale,
    bottom: (viewport.top + viewport.height - frameRect.top) / safeScale,
  };
}

window.__getZoomFrameOverlayMetrics = getZoomFrameOverlayMetrics;
...
let pendingViewportScrollAnchor = null;
...
function restoreViewportScrollAnchor(anchor) {
  if (!anchor || !anchor.scale) return;

  const nextScale = getCurrentPageScale();
  if (!Number.isFinite(nextScale) || nextScale <= 0) return;
  if (Math.abs(nextScale - anchor.scale) < 0.0001) return;
  ...
  window.scrollTo(anchor.x, nextY);
}
...
function applyViewportMetrics() {
  const scrollAnchor = pendingViewportScrollAnchor;
  pendingViewportScrollAnchor = null;
  viewportUpdateFrame = null;
  syncViewportMetrics();
  restoreViewportScrollAnchor(scrollAnchor);
...
function markViewportChanging() {
  ...
  if (!pendingViewportScrollAnchor) {
    pendingViewportScrollAnchor = captureViewportScrollAnchor();
  }
...
function positionDateCalendar(field, options) {
  ...
  if (frame) {
    const metrics = getZoomFrameOverlayMetrics(frame);
    if (!metrics) return;

    let left = (rect.left - metrics.frameRect.left) / metrics.scale;
    const belowTop = (rect.bottom - metrics.frameRect.top) / metrics.scale + gap;
    const aboveTop =
      (rect.top - metrics.frameRect.top) / metrics.scale - calendarHeight - gap;
    ...
    if (left + calendarWidth > metrics.right - viewportGap) {
      left = metrics.right - calendarWidth - viewportGap;
    }
    if (top + calendarHeight > metrics.bottom - viewportGap) {
      top = metrics.bottom - calendarHeight - viewportGap;
    }

    calendar.style.position = "absolute";
    calendar.style.left =
      Math.round(Math.max(metrics.left + viewportGap, left)) + "px";
    calendar.style.top =
      Math.round(Math.max(metrics.top + viewportGap, top)) + "px";
    return;
  }
...
function positionTooltip(tooltip, event) {
  ...
  const viewport = getVisualViewportRect();
  const pad = designPxToViewportPx(8);
  const gap = designPxToViewportPx(25);
  const left = clampOverlayValue(
    event.clientX - tooltip.offsetWidth / 2,
    viewport.left + pad,
    viewport.left + viewport.width - tooltip.offsetWidth - pad,
  );
  const top = clampOverlayValue(
    event.clientY + gap,
    viewport.top + pad,
    viewport.top + viewport.height - tooltip.offsetHeight - pad,
  );
...
function positionTooltipByElement(tooltip, element) {
  ...
  const viewport = getVisualViewportRect();
  let viewportLeft = rect.left + rect.width / 2 - tooltipW / 2;
  let viewportTop = rect.bottom + gap;

  viewportLeft = clampOverlayValue(
    viewportLeft,
    viewport.left + viewportPadding,
    viewport.left + viewport.width - tooltipW - viewportPadding,
  );

  if (viewportTop + tooltipH > viewport.top + viewport.height - viewportPadding) {
    viewportTop = rect.top - tooltipH - gap;
  }
  viewportTop = clampOverlayValue(
    viewportTop,
    viewport.top + viewportPadding,
    viewport.top + viewport.height - tooltipH - viewportPadding,
  );
